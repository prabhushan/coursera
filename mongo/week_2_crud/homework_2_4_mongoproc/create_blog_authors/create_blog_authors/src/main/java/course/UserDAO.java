...
public class UserDAO {
    ...
    public boolean addUser(String username, String password, String email) {
        ...
        Document addUserDocument = new Document().append("_id",username).append("password", passwordHash);
        if (email != null && !email.equals("")) {
            // XXX WORK HERE
            // if there is an email address specified, add it to the document too.
        	addUserDocument.append("email", email);
        }

        try {
            // XXX WORK HERE
            // insert the document into the user collection here
        	usersCollection.insertOne(addUserDocument);
            return true;
    ...
    public Document validateLogin(String username, String password) {
        ...
        List<Document> listUsers = usersCollection.find(filter).into(new ArrayList<Document>());
